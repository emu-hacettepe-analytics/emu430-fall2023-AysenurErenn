...
ggplot(average_ratings, aes(x = Years, y = Number_of_Movies)) +
  ...
  labs(title = "urkish Movies Released Each Year",
       x = "Year",
       y = "Number of Movies") +
  theme_minimal()

# Box plots of ratings over the years
ggplot(movies, aes(x = as.factor(Years), y = Ratings)) +
  geom_boxplot(fill = "blue") +
  labs(title = "Box Plots of Ratings of Movies Over the Years",
       x = "Year",
       y = "Rating") +
  theme_minimal()
```
...
```{r}
votes_rating <- cor(movies$Votes, movies$Ratings)

cat("Correlation Coefficient between Votes and Ratings:", votes_rating, "\n")
```

#e-)

```{r}
duration_rating <- cor(movies$Durations, movies$Ratings)

cat("Correlation Coefficient between Duration and Ratings:", duration_rating, "\n")
```
